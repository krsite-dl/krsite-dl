...
def download_handler_naver(img_list, dirs, post_date):
    ...
    for img in img_list:
        ...
        local_dt = datetime.datetime.strptime(post_date, "%Y.%m.%d. %H:%M")
        utc_dt = local_dt.astimezone(pytz.utc)
        
        utc_offset = utc_dt.strftime("%z")
        utc_offset_hours = int(utc_offset[:-2])
        offset = datetime.timedelta(hours=utc_offset_hours)


        if img_name != os.path.exists(paths):
            subprocess.run(['aria2c', img, '-d', dirs, '--continue=true'])
            subprocess.run(['exiftool', paths, f'-FileModifyDate+=0{utc_offset_hours}:00'])
